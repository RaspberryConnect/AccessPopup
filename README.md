...
If the web app become unresponsive but you know the ACPU_Device and Phone are on the same network, then there may be an issue. You can restart the web server by using the installerconfig script. Use option 8-1 “Web Interface - enable & disable switch” to disable the Web App and then again to re-enable the Web App.
